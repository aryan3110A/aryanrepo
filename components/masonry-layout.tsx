...
export default function MasonryLayout({ images, onImageClick, onLikeToggle, onBookmarkToggle }: MasonryLayoutProps) {
  const [hoveredImageId, setHoveredImageId] = useState<string | null>(null)
  const [imageDimensions, setImageDimensions] = useState<Record<string, { width: number; height: number }>>({})
  const [imagesLoaded, setImagesLoaded] = useState(false)
  const [modifiedImages, setModifiedImages] = useState<ArtImage[]>([])
  const containerRef = useRef<HTMLDivElement>(null)
  const [columns] = useState(3)

  useEffect(() => {
    // Update images with bookmark status from store
  ...
  const handleBookmarkToggle = (image: ArtImage) => {
    ...
    const updatedImages = modifiedImages.map((img) =>
      img.id === image.id ? { ...img, bookmarked: newBookmarkedState } : img,
    )
    setModifiedImages(updatedImages)
